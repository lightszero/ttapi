...
export class AniPlayer {
    ...
    constructor(ttani: TTAni, adapter: IAniPlayerAdapter) {
        ...
        //初始化Ani
        for (var i = 0; i < this.ani.frame.length; i++) {
            let f = this.ani.frame[i];
            if (f.pics != null) {
    ...
    Update(delta: number) {
        ...
        while (this.curframe >= this.ani.frame.length) {
            if (this.ani.loop) {
                this.curframe -= this.ani.frame.length;
            }
            else {
                this.curframe = this.ani.frame.length - 1;
            }
        }
        let fid = this.curframe | 0;
        //console.log("curframe=" + fid);
        
        let frame = this.ani.frame[fid];
        if (frame.refframe >= 0)
            frame = this.ani.frame[frame.refframe];
