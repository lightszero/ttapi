...
import { ISceneSystem } from "../system/scenesystem_box2d.js";

import { ISceneRender, ISceneRenderItem, SceneNode } from "./scenenode.js";
...
export class Scene implements ILayerRender {
    ...
    AddRenderItem(item: ISceneRenderItem): void {
        if (item.sort) {
            this.renderList_Sorted.push(item);
        }
        else {
            this.renderList_NoOrder[item.type].push(item);
        }
    }
