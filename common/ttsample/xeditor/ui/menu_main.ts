import { IOExt, QUI_Canvas, QUI_Direction2, QUI_Grow, QUI_Menu, QUI_Panel, QUI_Resource } from "../../../ttlayer2/ttlayer2.js";
...
export class Menu_Main {
    static Init(canvas: QUI_Canvas, quit: () => void) {
        let menu: QUI_Menu;
        ...
        );
        {
            ...
            if (quit != undefined) {
                submenuFile.items.push
                    (
                        {
                            label: "退出编辑器", sprite: QUI_Resource.GetSprite("border"), submenu: null, onaction: () => {
                                quit();
                            }
                        }
                    );
